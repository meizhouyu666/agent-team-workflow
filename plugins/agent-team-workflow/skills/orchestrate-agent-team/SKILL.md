...
Use `[ATW][Executor][<project-dir>]` as this role's `launch_task.title` and TaskBinding `title`. Make it the first prompt line for a new session, and reuse it after resume or rebind. When authorized to launch the Reviewer, use `[ATW][Reviewer][<project-dir>]` in the same fields. Never verify authority from either title.
...
For a replacement request, checkpoint state, launch one replacement using the user's current Provider configuration and role title, verify identity and reconciliation, rebind with the same display title, and only then stop the old process. Failed verification leaves the old process alive. Whole-team rotation order is Executor, Reviewer, Leader last; never execute it unless authorized by the Leader and user.
